...
import numpy as np
from .utils import BaseSequence, requires_all_loaded
...
class BatchSequence(BaseSequence):
    ...
    def _get_periods_in_range(self, sleep_study, start, end):
        ...
        """
        print(f"ss: {sleep_study}, start={start}, end={end}")
        X, y = [], []
...
# ----------------------------------------------------------------------------- 
class SleepStudyDatasetPytorch(torch.utils.data.Dataset, BaseSequence):
    """ Pytorch wrapper of SleepStudyDataset 
    """
    def __init__(self,
                 sleep_study_pairs,
                 batch_size,
        ...
        """
        Args:
            sleep_study_pairs (list):
                A list of SleepStudy objects
            batch_size (int):
                The size of each sampled batch
            data_per_period (int): The dimensionality/number of samples in each 
                'period/epoch/segment' of data
            n_classes (int):
                Number of classes (sleep stages)
            n_channels (int):
                The number of PSG channels to expect in
                data extracted from a SleepStudy object
            margin (int):
                The margin (number of periods/segments) to include down- and 
                up-stream from a selected center segment. E.g. a margin of 3 
                will lead to 7 connected segments being returned in a sample.
            batch_scaler (string):
                The name of a sklearn.preprocessing Scaler object to apply to
                each sampled batch (optional)
            logger (Logger):
                A Logger object
            no_log (bool):
                Do not log information on this Sequence
            identifier (string):
                A string identifier name
        """
        self._inferred = n_classes is None
        n_classes = _infer_n_classes(n_classes, sleep_study_pairs)
        super().__init__(identifier=identifier,
    ...
    def get_pair_by_id(self, study_id):
        ...
        Returns:
            A stored SleepStudy object
        Note:
            Copied from https://github.com/getty708/U-Time-PyTorch/blob/500998103eebb1360aacaf6a894f3c679d58bdc2/src/utime/sequences/batch_sequence.py#L219
        """
        return self.id_to_pair[study_id]        
    ...
    def get_period(self, study_id, period_idx, allow_shift_at_border=True,
        ...
        Returns:
            ...
            y, list of margin*2+1 class labels
        Note:
            Copied from https://github.com/getty708/U-Time-PyTorch/blob/500998103eebb1360aacaf6a894f3c679d58bdc2/src/utime/sequences/batch_sequence.py#L310
        """
        margin = margin or self.margin
        sleep_study = self.get_pair_by_id(study_id)
        n_periods = sleep_study.n_periods
